...
import { getAllBlogs, deleteBlog, exportBlogs } from '../../../store/actions/blogActions';
...
function Blogs() {
  ...
  const blogs_export = useSelector(state => state.blog.blogsExport);
  ...
  const exportToExcel = () => {
    exportFromJSON({ data: blogs_export.blogs, fileName, exportType });
  };
  ...
  const fetchSearchResults = async () => {
    ...
    setErrorMessage(false);
    try {
      await dispatch(getSearchResult('blogs', searched, 1, ''));
      setLoadingData(false);
    } catch (error) {
      const err = error.response.data.message;
      console.log('fff', error.response.data.message);
  ...
  return (
    <div>
      <PageHeader title="Blogs" withButton buttonText="New " buttonIcon="fa-plus" buttonLink="/blog/new" />
      {loadingData ? (
        <Loader type="bars" />
      ) : (
                      ...
                      <tr key={blog._id}>
                        ...
                        <td>
                          <Link
                            ...
                            onClick={() => localStorage.setItem('id', blog._id)}
                          >
                            {blog.title}
                          </Link>
                        </td>
                        ...
                        <td>
                          <div className="btn-group">
                            ...
                            <div className="dropdown-menu">
                              <UncontrolledDropdown>
                                <DropdownItem>
                                  <Link className="primary-color-text" to={`/blog/view/${blog._id}`}>
                                    View
                                  </Link>
                                </DropdownItem>
                                {isSuperAdmin() && (
